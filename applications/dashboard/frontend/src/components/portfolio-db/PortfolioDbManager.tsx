...
/** RR = (TP - Entry) / (Entry - SL). Returns null when any input is null or denominator is zero. */
const calcRR = (entryPrice: number | null, sl: number | null, tp: number | null): number | null => {
  if (entryPrice == null || sl == null || tp == null) return null
  const denom = entryPrice - sl
  if (denom === 0) return null
  return (tp - entryPrice) / denom
}

const fmtRR = (rr: number | null): string =>
...
export function PortfolioDbManager() {
  ...
  const handleUpdate = async (input: PositionInput) => {
    ...
    setMutating(true)
    try { await portfolioDbService.update(editTarget.id, input); setEditTarget(null); await invalidate(); toast.success('Position updated') }
    catch (e: any) { toast.error(e?.response?.data?.detail ?? 'Failed to update position') }
    finally { setMutating(false) }
  }
  ...
  const handleDelete = async () => {
    ...
    setMutating(true)
    try { await portfolioDbService.delete(deleteTarget.id); setDeleteTarget(null); await invalidate(); toast.success('Position deleted') }
    catch (e: any) { toast.error(e?.response?.data?.detail ?? 'Failed to delete position') }
    finally { setMutating(false) }
  }

  const handleUndo = async (pos: DbPosition) => {
    setUndoing(pos.id)
    try { await portfolioDbService.undoSell(pos.id); await invalidate(); toast.success('Sell undone') }
  ...
  const orphanChildren = positions.filter(p => p.parentId && !childMap[p.parentId])
          ...
          <table className="w-full text-xs" style={{ minWidth: 1080 }}>
            <thead>
              <tr className="border-b border-border/50 bg-surface-elevated/40 text-ink-muted">
                {['SYMBOL', 'DIR', 'ENTRY DATE', 'ENTRY ฿', 'CURRENT / EXIT ฿', 'SIZE', 'NET P&L', 'P&L %', 'SL', 'TP', 'RR', 'STATUS', 'CREATED', 'MODIFIED', ''].map(h => (
            ...
            <tbody>
              {isLoading ? (
                <tr><td colSpan={15} className="px-3 py-10 text-center text-ink-muted">
                ...
                return (
                  <tr key={pos.id} className={cn(
                    ...
                    <td className="px-2.5 py-2 tabular-nums text-ink-secondary">
                      {fmtRR(calcRR(pos.entryPrice, pos.sl, pos.tp))}
                    </td>
                    <td className="px-2.5 py-2">
